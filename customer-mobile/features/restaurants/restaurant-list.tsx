...
export function RestaurantList({ restaurants, refreshing, onRefresh, onSelect, onEndReached, loadingMore, hasMore }: Props) {
  return (
    <ScrollView
      style={styles.scroll}
      ...
      onScroll={onEndReached ? ({ nativeEvent }) => {
        ...
        if (closeToBottom && hasMore && !loadingMore) onEndReached();
      } : undefined}
  ...
  return (
    <Pressable accessibilityRole="button" onPress={onPress} style={({ pressed }) => [styles.card, pressed && styles.pressed]}>
      ...
      <View style={styles.cardBody}>
        <View style={styles.cardHeading}><Text numberOfLines={1} style={styles.name}>{restaurant.name}</Text><Text style={styles.rating}>★ {Number(restaurant.rating).toFixed(1)}</Text></View>
